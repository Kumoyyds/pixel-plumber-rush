...
export class HUD {
  draw(ctx, gameState = {}) {
    ...
    const hearts = Math.max(0, Number(state.hearts ?? state.lives ?? 0));
    const maxHearts = Math.max(hearts, Number(state.maxHearts ?? 3));
    const score = Math.max(0, Number(state.score) || 0);
    const coins = Math.max(0, Number(state.coins) || 0);
    const combo = Math.max(1, Number(state.coinCombo ?? state.multiplier ?? 1));
    const checkpoint = state.checkpoint ?? "START";
